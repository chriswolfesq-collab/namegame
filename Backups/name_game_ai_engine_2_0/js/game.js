...
async function startGame(){
 ...
 if(window.AIEngine){
   ...
   await AIEngine.prepare({category:gameCategory, difficulty:aiDiff, knownNames:aiNames});
 }
...
function aiTakeTurn(turnId,playerIndex){
 ...
 let person=aiPickFromDatabase(diff);

 // True fallback only when the database engine is not ready at all.
 // If Baseball loaded, Baseball uses the JSON database only.
 if(!person && (!window.AIEngine || !AIEngine.ready)){
   const fallbackName = aiPickFallback(diff);
   if(fallbackName) person = {id:fallbackName.toLowerCase(), name:fallbackName};
 }
 ...
 const thinkRange={Easy:[1400,3000],Medium:[900,1900],Hard:[650,1300],Expert:[450,950]}[diff]||[900,1900];
