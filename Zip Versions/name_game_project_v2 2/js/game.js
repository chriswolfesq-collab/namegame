...
function aiPoolForCurrentCategory(){
...
function aiTakeTurn(turnId,playerIndex){
 ...
 const valid=aiPoolForCurrentCategory().filter(n=>{
 ...
 if(Math.random()<failRate || !valid.length){
  ...
  try{
    ...
    if(res.ok){
      ...
      if(data && data.title && data.type !== 'disambiguation'){
        ...
        const wikiResult={valid:true, title:data.title, description:detail, photo:data.thumbnail&&data.thumbnail.source?data.thumbnail.source:'', categoriesText:data.categoriesText||'', url:data.content_urls && data.content_urls.desktop ? data.content_urls.desktop.page : ''}; if(window.NameDatabase){NameDatabase.cacheWiki(name,wikiResult);} return wikiResult;
      }
    }
    ...
    if(best && best.title){
      ...
      const wikiResult={valid:true, title:best.title, description:best.description || '', photo:'', categoriesText:best.categoriesText||'', url:best.fullurl||''}; if(window.NameDatabase){NameDatabase.cacheWiki(name,wikiResult);} return wikiResult;
    }
...
// Phase 1 database preload. Game still works if fetch is blocked by file://.
if(window.NameDatabase){ NameDatabase.loadAll().catch(()=>{}); }
